...
Every link import also attempts a real 1280×960 browser screenshot in parallel with AI web search. The screenshot is separate evidence of what an unsigned Chromium browser reached, not proof of what the search provider could read. The app exposes a private View screenshot link on the recipe and in Agent activity, including the page title, HTTP status when available, and incomplete-navigation notice. Failed captures are reported rather than replaced with a generated image. Captures do not sign in, bypass access challenges, or play videos.

Screenshots use bundled Chromium and Puppeteer on Node 24, with a 28-second capture deadline. Outbound browser traffic uses a per-capture proxy that resolves and pins public IPs, rejecting private addresses and non-web ports. JPEGs are stored separately from household snapshots in Postgres, restricted to signed-in household users, and retained for seven days with a maximum of 50 images. Deleted recipe screenshots cannot be served; expired rows are cleaned on the next successful capture. No additional browser service account is required. Capture page again starts a screenshot-only job without an OpenAI request or changes to recipe details; it shares the daily job allowance.
