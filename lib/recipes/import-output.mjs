...
export const recipeSchema=object({name:string,tags:{type:'array',items:string},servings:number,ingredients:{type:'array',items:object({name:string,quantity:number,unit:string,section:{type:'string',enum:['Produce','Dairy & eggs','Meat & fish','Pantry','Other']}})},steps:{type:'array',items:string},sourceAccessed:{type:'boolean'},accessStatus:{type:'string',enum:['readable','login-required','video-only','not-found','unknown']},message:string});
