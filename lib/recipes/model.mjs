...
export function webUrl(value) {
  try {
    ...
    if (
      ...
      return "";
    u.hash = "";
    ["igsh", "utm_source", "utm_medium", "utm_campaign"].forEach((k) =>
...
export const urlKey = (v) =>
  webUrl(v).replace(/\/$/, "").replace("/reels/", "/reel/");
...
export function applyAction(state, action) {
  ...
  switch (action.type) {
    case "save": {
      ...
      if (!input.url) throw new Error("Please enter a valid recipe link.");
      const duplicate = state.recipes.find(
        (x) => x.id !== input.id && urlKey(x.url) === urlKey(input.url),
      );
      ...
      if (old) {
        ...
        Object.assign(old, input, { revision: old.revision + 1 });
      } else {
        if (
          !input.id ||
    ...
    case "migrate": {
      ...
      for (const recipe of legacy.custom || []) {
        if (
          !state.recipes.some(
            (x) => x.id === recipe.id || urlKey(x.url) === urlKey(recipe.url),
          )
        )
