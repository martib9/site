...
export function applyTelegramUpdate(state, update, { allowImport = true, configured = true, now = Date.now() } = {}) {
  ...
  const tg = telegramState(state), chatId = String(m.chat.id);
  ...
  if (pair) {
    ...
    if (!user) tg.users.push({ id: chatId, name: text(m.from.first_name) || 'Household member', connectedAt: now });
    return { chatId, reply: 'Connected to your household recipes. Send one recipe link per message. Add a caption or #breakfast, #lunch or #dinner if you like.' };
  }
  ...
  const { url, multiple } = messageLink(m);
