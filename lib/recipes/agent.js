...
export async function runJob(id) {
  ...
  try {
    if (claim.job.kind === "capture") {
      ...
      // Both attempts finish before the job ends, including when AI extraction fails.
      const [extraction] = await Promise.allSettled([research(
        ...
        undefined,
        !job.caption,
      ), captureEvidence(id, recipe.url)]);
      if (extraction.status === "rejected") throw extraction.reason;
      const { value, urls } = extraction.value;
      const hasEvidence =
        Boolean(job.caption) ||
